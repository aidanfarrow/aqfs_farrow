...
setenv LD_LIBRARY_PATH "${LD_LIBRARY_PATH}:/home/chemel/public/netcdf-v4.2.1.1/gfortran/lib"
...
sed -e "s/@FFDate@/$cFDATE/g" meteo.exe.run.xavier > meteo.exe.run.tmp;
/usr/local/bin/qsub meteo.exe.run.tmp

sleep 2100

sed -e "s/@FFDate@/$cFDATE/g" mcip-megan.exe.run.xavier > mcip-megan.exe.run.tmp;
./mcip-megan.exe.run.tmp >& mcip-megan.log &

sleep 240
sed -e "s/@FFDate@/$cFDATE/g" all.exe.run.xavier > all.exe.run.tmp;
